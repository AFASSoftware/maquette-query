...
describe("query", () => {
  ...
  describe("selector", () => {
    ...
    it("can find an element by className", () => {
      ...
      expect(createQuery(tree).query(".classA")).to.deep.include({
    ...
    it("can find an element by tagname", () => {
      ...
      expect(createQuery(tree).query("span")).to.deep.include({
        vnodeSelector: "span.classA.classB#id1",
      });
    });
    ...
    it("can find an element by id", () => {
      ...
      expect(createQuery(tree).query("#id1")).to.deep.include({
        vnodeSelector: "span.classA.classB#id1",
      });
    });
  ...
  describe("textContent", () => {
    ...
    it("returns text segments of a vnode and its descendants", () => {
      expect(createQuery(h("p", ["A ", h("i", ["funny"]), " tale"])).textContent).to.equal(
        "A funny tale"
      );
    });
  });
